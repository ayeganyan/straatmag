...
async function getVendor(vendorId: VendorUUID): Promise<Vendor> {
  ...
  if (!vendorDoc) {
    return Promise.reject(new Error(`Vendor with id ${vendorId} does not exist`));
  }
...
async function getVendorDocById(vendorId: VendorUUID): Promise<QueryDocumentSnapshot<Vendor>> {
  const queryVendorById = query(vendorsRef, where('id', '==', vendorId)).withConverter(vendorConverter);

  const vendorsSnapshot = await getDocs(queryVendorById);
  const doc = vendorsSnapshot.docs.at(0);

  return doc ? doc : Promise.reject(new Error(`Vendor with id ${vendorId} does not exist`));
}
