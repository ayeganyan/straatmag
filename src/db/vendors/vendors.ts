...
async function getVendorByRFID(rfId: Rfid): Promise<Vendor> {
  ...
  return vendorSnapshot ? vendorSnapshot.data() : Promise.reject(new Error(`Vendor with rfid "${rfId}" does not exist`));
}
...
async function getVendorByName(name: string): Promise<Vendor> {
  ...
  return vendorsSnapshot ? vendorsSnapshot.data() : Promise.reject(new Error(`Vendor with name "${name}' does not exist`));
}
